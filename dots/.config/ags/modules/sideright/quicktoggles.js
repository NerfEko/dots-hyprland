...
export const ModuleProtonVPN = async (props = {}) => {
    if (!exec(`bash -c 'command -v nmcli'`)) return null;

    const getActiveServer = () => {
        const out = exec(`bash -c "nmcli -t -f NAME,TYPE connection show --active 2>/dev/null | awk -F: '\\$2==\"wireguard\" && \\$1~/^ProtonVPN /{print \\$1}' | head -1"`);
        return (out || '').trim();
    };

    const getMostRecent = () => {
        const out = exec(`bash -c "nmcli -t -f NAME,TYPE,TIMESTAMP connection show 2>/dev/null | awk -F: '\\$2==\"wireguard\" && \\$1~/^ProtonVPN /{print \\$1 \":\" \\$3}' | sort -t: -k2 -rn | head -1 | cut -d: -f1"`);
        return (out || '').trim();
    };

    return Widget.Button({
        className: 'txt-small sidebar-iconbutton',
        tooltipText: getString('ProtonVPN | Right-click for servers'),
        onClicked: (self) => {
            ...
            if (active) {
                ...
                if (recent) {
                    execAsync(['bash', '-c', `nmcli connection up '${recent}'`]).catch(print);
                    self.toggleClassName('sidebar-button-active', true);
                }
            }
        },
        onSecondaryClickRelease: () => {
            // Switch to VPN panel — callback registered by sideright.js after init
            if (globalThis.__sidebarShowVpn) globalThis.__sidebarShowVpn();
            App.openWindow('sideright');
        },
        child: MaterialIcon('shield', 'norm'),
        setup: (self) => {
            setupCursorHover(self);
            // Poll every 3s to reflect connection state
            self.poll(3000, (btn) => {
                const active = getActiveServer();
                btn.toggleClassName('sidebar-button-active', !!active);
                btn.tooltipText = active
                    ? `${getString('ProtonVPN')}: ${active.replace('ProtonVPN ', '')} | ${getString('Right-click for servers')}`
                    : `${getString('ProtonVPN — disconnected')} | ${getString('Right-click for servers')}`;
            });
        },
        ...props,
    });
...
export const ModuleIdleInhibitor = (props = {}) => Widget.Button({
    ...
    setup: (self) => {
        setupCursorHover(self);
        self.poll(1000, (btn) => {
            const isRunning = !!exec('pidof wayland-idle-inhibitor.py');
            btn.attribute.enabled = isRunning;
            btn.toggleClassName('sidebar-button-active', isRunning);
        });
    },
    ...props,
});
